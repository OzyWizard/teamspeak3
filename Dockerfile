...
RUN mkdir -p /var/ts3server
RUN chmod 777 /var/ts3server
...
RUN tar -xf server.tar.bz2 --strip-components=1 -C /var/ts3server
RUN rm server.tar.bz2

RUN wget "${DB_FILE}" -O /var/ts3server/ts3db_mariadb.ini
RUN wget "${INI_FILE}" -O /var/ts3server/ts3server.ini
RUN wget "${START_SCRIPT}" -O /var/ts3server/ts3server_startscript.sh
...
CMD ["/var/ts3server/ts3server_startscript.sh"]
